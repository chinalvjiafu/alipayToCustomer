<?php
namespace chinalvjiafu\alipaytocustomer\Alipay\aop;
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		if ("json" == $this->format) {
			...
			if (null !== $respObject) {
				...
				//返回结果错误
				$keyArray = array('alipay.fund.trans.order.query' => 'alipay_fund_trans_order_query_response' ,'alipay.fund.trans.toaccount.transfer'=> 'alipay_fund_trans_toaccount_transfer_response');	
				if (isset($keyArray[$sysParams["method"]])) {
					$key = $keyArray[$sysParams["method"]];
					if ( 10000 != $respObject->$key->code) {
						return $respObject;
					}
				}
